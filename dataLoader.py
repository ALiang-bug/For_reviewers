...
class datasets(Dataset):
    def __init__(self, data, cas_len, device="cuda"):
        ...
        self.processed_data = self._preprocess_all_data()

    def _preprocess_all_data(self):
        processed = []
        for idx in range(self.cas_num):
            inp = self.cas_np[idx]
            inp_t = self.cas_time[idx]
            ...
            n_nonzero = len(nonzero_elems)

            pad_len = self.max_len - n_nonzero
            cascades = torch.tensor(inp, dtype=torch.long, device=self.device)
            if pad_len > 0:
                pad = torch.zeros(pad_len, dtype=torch.long, device=self.device)
                cascades = torch.cat([cascades, pad])

            cascades_time = torch.tensor(inp_t, dtype=torch.float32, device=self.device)
            if pad_len > 0:
                pad_t = torch.full((pad_len,), inp_t[-1], dtype=torch.float32, device=self.device)
                cascades_time = torch.cat([cascades_time, pad_t])

            cascade_len = torch.tensor(n_nonzero, dtype=torch.long, device=self.device)
            if cascade_len == self.max_len:
                cascade_len = torch.tensor(self.max_len - 1, dtype=torch.long, device=self.device)

            cascades_in = cascades[:-1]
            cascades_out = cascades[1:]
            cascades_time_in = cascades_time[:-1]
            cascades_time_out = cascades_time[1:]

            processed.append((cascades_in, cascades_out, cascades_time_in, cascades_time_out, cascade_len))
